...
public class BookShareAppBut{
    ...
    public static long currentphone;
  ...
  public static void delbook(){
   ...
   public static void plusMember(){
    ...
    while(ct) {
     ...
     ct = true;
			while(ct) {
				...
				for(int x = 0; x < currentemail.length(); x++) {
					String temp11 = currentemail.charAt(x) + "";
					if(temp11.equals("@")) {
						if(currentemail.substring(x+1,currentemail.length()).equals("gmail.com") || currentemail.substring(x+1,currentemail.length()).equals("yahoo.com")) {
							ct = false;
						}
					}
				}
				if(ct) {
					System.out.println("Please enter a valid gamil or yahoo email");
				}
			}
    ...
    public static void main(String[] args){
        BookShareAppBut bd = new BookShareAppBut(400,130);
        bd.setup();
        bd.setListen();
